...
function mapMosaicStream (style, mapDiv, opts) {
  ...
  var tiles = getTileStreams(viewport, dpDim, maxTileDim, style, bbox, opts.debug)

  var mapMosaic = mosaic(tiles, pixelDim[1])
  ...
  //pixel width and height x 4 colour chanels
  var total = pixelDim[0] * pixelDim[1] * 4
  var complete = 0

  const proggressMonitor = spy((chunk) => {
    complete += chunk.length
    out.emit('progress', complete / total, total)
  })


  var out = pumpify(mapMosaic, proggressMonitor, pngStream)

  pngStream.on('format', (format) => {
    out.emit('format', format)
  })

  return out
}

function getTileStreams (viewport, dim, maxTileDim, style, origBbox, debug) {
  ...
  return Array(cols).fill(1).map(function (_, i) {
    ...
    return function createStream (cb) {
      ...
      if (map.loaded()) {
        cb(null, mapImageStream(map, [width, height], [w, s, e, n], origBbox, debug))
      } else {
